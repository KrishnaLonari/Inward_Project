...
public class SingXIntegrations {
	...
	public SingXIntegrations() {
		...
		kongSource 					= EnvProperties.getPropertyValue("KongSource") == null ? "DIGIREMIT_INWARD": EnvProperties.getPropertyValue("KongSource");
	...
	public int insertSingXDetails(ArrayList singXData) {
		...
		try {
			...
			insQry.append("			loginId,						");// 5
			insQry.append("			corridorName,					");// 6
			insQry.append("			createdDate  )	VALUES	(		");// 7
			insQry.append("		      ?,?,							");
			insQry.append("		      ?,?,			    			");
			insQry.append("		      ?,?,?,		    				");
			...
			paramTypes.add(0);
			params.add((String) singXData.get(2));
			paramTypes.add(0);
			params.add((String) singXData.get(6));
			paramTypes.add(0);
	...
	public int updateSingXDetails(ArrayList singXData) {
		...
		try {
			...
			updQry.append("		errorDesc = ?,			");
			updQry.append("		loginId = ?,			");
			updQry.append("		userId = ?,				");
			updQry.append("		corridorName = ?				");
			updQry.append(" WHERE						");
			...
			paramTypes.add(0);
			params.add((String) singXData.get(1)); // loginId
			paramTypes.add(0);
			params.add((String) singXData.get(11)); // errorDesc
			paramTypes.add(0);
			params.add((String) singXData.get(1)); // loginId
			paramTypes.add(0);
			params.add((String) singXData.get(3)); // userId
			paramTypes.add(0);
			params.add((String) singXData.get(12)); // txnSXId
			paramTypes.add(0);
			params.add((String) singXData.get(14)); // corridorName
			paramTypes.add(0);
		...
		try {
			...
			case "Register":
				if ("AU".equalsIgnoreCase(country)) {
			        documents = new ArrayList<>(); // Ensure it's empty
			    }
	...
	public String generateExistingCustRequest(HashMap<String, String> singXRequestHm,
		...
		if (country.equals("SG") || country.equals("HK")) {
			...
			request = "{" + "\"email\":\"" + singXRequestHm.get("email") + "\"" + "}";
		}else{
			Logger.log("other country : \n" + country, FILENAME, LOGNAME, null, Logger.INFO);
		}
	...
	private HashMap<String, String> readExistingCustResponse(String fullResponse, String country) {
		 ...
		 * {
		    ...
		    "success": true,
		    "customerId": "ashok.kumar@singx.co",
		    "status": "ONBOARDED"
		}		

		 {"existingCustomer":false,"success":true}
		 */
		
		/*
		 * AU api response-{ "existingCustomer": true, "success": true, "customerId":
		 * "ashok.kumar@singx.co", "status": "ONBOARDED" }
		 */
		...
		try {
			...
			success = resJsonObj.has("success") ? String.valueOf(resJsonObj.get("success")) : "";
			...
			if (country.equals("SG") || country.equals("HK")) {
				...
				//isExist = resJsonObj.getString("existingCustomer");
				isExist = resJsonObj.has("existingCustomer") ? String.valueOf(resJsonObj.get("existingCustomer")) : "";
	...
	private String excuteRequest(String targetURL, String requestEntity, String method, String ContentType) {
		...
		try {
			...
			connection.setRequestProperty("source", kongSource);
	...
	private String excuteEncryptedRequest(String targetURL, String signatureURL, String requestEntity, String method, String ContentType) {
        ...
        try {
	       ...
	       // timestamp = (System.currentTimeMillis() / 1000) - (30 * 60) - 10;
...
//					Logging all headers of connection -->
	...
	public String generateOTPRequest(HashMap<String, String> signXRequest) {
		...
		if (country.equals("SG") || country.equals("HK")) {
			...
			jsonRequest.append( "{" ).append("\"source\":\"").append(source).append( "\"," ).append("\"customerId\":\"" ).append(customerId).append("\"" ).append("}");
			ekycRequest =jsonRequest.toString(); 
		}
		else if (country.equals("AU")) {
			String customerId = signXRequest.get("customerId") == null ? "" : signXRequest.get("customerId").toString();
			jsonRequest.append( "{" ).append("\"customerId\":\"" ).append(customerId).append("\"" ).append("}");
			ekycRequest =jsonRequest.toString(); 
		}
	...
	private HashMap<String, String> readGenerateOTPResponse(String fullResponse, String country) {
		...
		/*
		 * for the AU generate otp response { "message": "OTP Sent Successfully",
		 * "total": 0, "failtokens": null, "notificationFailed": 0,
		 * "notificationSuccess": 0, "status": 200 }
		 */
...
//	Register API starts -->
	...
	private HashMap<String, String> readCustRegResponse(String fullResponse, String country) {
		 ...
		 * 
					 * {
			    "success": true,
			    "customerId": "testOnboarding0604202501@singx.co",
	...
	private HashMap<String, String> readCustStatusResponse(String fullResponse, String country) {
		...
		try {
			...
			if (country.equals("SG") || country.equals("HK")) {
				...
				status = resJsonObj.has("Onboarded") ? String.valueOf(resJsonObj.get("Onboarded")) : "";
				auStatus = resJsonObj.has("success") ? resJsonObj.getInt("success") : 0;
...
//	Cust Details API Starts -->
	...
	public String generateCustDtlsRequest(HashMap<String, String> signXRequest) {
		...
		if (country.equals("SG") || country.equals("HK")) {
			...
			jsonRequest.append("{")
						.append("\"customerId\":\"")
						.append(signXRequest.get("customerId"))
						.append("\"}");
	...
	private HashMap<String, String> readCustDtlsResponse(String fullResponse, String country) {
		...
		try {
			resJsonObj = new JSONObject(fullResponse);
			if (country.equals("SG") || country.equals("HK")) {
				token = resJsonObj.getString("token");
				...
				Logger.log("readCustDtlsResponse else block : \n", FILENAME, LOGNAME, null, Logger.INFO);
			}
